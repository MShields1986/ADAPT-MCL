#!/usr/bin/env python3
"""
Build VPR database using MixVPR descriptors from all 3 cameras.
...
At query time, querying with ANY camera will find the nearest descriptor
regardless of which camera produced it.

Usage:
    docker compose run --rm vpr scripts/vpr/build_database_mixvpr_multicam.py \
...
def main():
    ...
    # db_entries: list of {pose, camera, descriptor, keyframe_idx}
    db_entries = []
    all_descs = []
    ...
    all_descs = np.vstack(all_descs)  # (3*N, desc_dim)
    ...
    # Save database with descriptors embedded in entries
