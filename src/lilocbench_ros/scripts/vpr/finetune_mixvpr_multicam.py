#!/usr/bin/env python3
"""
Fine-tune MixVPR on the mapping sequence using all 3 cameras (front, left, right).
...
class MixVPRModel(nn.Module):
    ...
    def __init__(self, backbone_arch="resnet18", layers_to_freeze=0,
        ...
        print(f"MixVPR: {backbone_arch} -> {in_channels}ch x {in_h}x{in_w} -> "
...
class MultiCamTrajectoryDataset(Dataset):
    ...
    def __init__(self, data_dir, cameras, gt_path, transform, subsample=5):
        ...
        self.images = []
        self.poses = []
        self.cam_indices = []  # 0=front, 1=left, 2=right
        ...
        for i, fname in enumerate(front_files):
            ...
            # Find matching left/right images (within 50ms)
...
def mine_triplets(embeddings, poses, indices, pos_thresh=2.0, neg_thresh=10.0, min_gap=0):
    """Mine hard triplets. min_gap=0 allows cross-camera positives at same pose."""
    n = len(embeddings)
    ...
    for i in range(n):
        # Positives: within pos_thresh distance, not self
        ...
        # Hardest positive (furthest in embedding space among positives)
        pe = emb_dists[i].clone()
        pe[~pos_mask] = -1
        # Hardest negative (closest in embedding space among negatives)
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune MixVPR (multi-camera)")
