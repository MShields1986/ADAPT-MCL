#!/usr/bin/env python3
"""
Fine-tune MegaLoc on the mapping sequence using all 3 cameras.
...
Uses mixed precision to fit in GPU memory.
"""
...
def main():
    parser = argparse.ArgumentParser(description="Fine-tune MegaLoc (multi-camera)")
    ...
    parser.add_argument("--output", type=str,
                        default=os.path.join(MODELS_DIR, "megaloc_multicam_finetuned.pth"))
    args = parser.parse_args()

    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Device: {device}")

    print("Loading pretrained MegaLoc...")
    model = torch.hub.load("gmberton/MegaLoc", "get_trained_model",
                           trust_repo=True).to(device)

    # Freeze backbone (DINOv2), only train aggregator
    for name, param in model.named_parameters():
        if name.startswith("backbone"):
            param.requires_grad = False
    trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    total = sum(p.numel() for p in model.parameters())
    print(f"Total params: {total/1e6:.1f}M, trainable: {trainable/1e6:.1f}M (aggregator only)")
    ...
    print(f"\nTraining {args.epochs} epochs, bs={args.batch_size}, lr={args.lr}, mixed precision")
    best_loss = float("inf")
