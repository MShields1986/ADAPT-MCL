#!/usr/bin/env python3
"""
Generate VPR candidates for a sequence using multi-camera EigenPlaces.

Queries the database with all 3 cameras from the first frame of the test
sequence. Merges and deduplicates candidates across cameras.

Usage:
    docker compose run --rm vpr scripts/vpr/publish_vpr_candidates_eigenplaces_multicam.py \
        --sequence static_0 --top-k 10 --output-file /data/static_0/vpr_candidates_eigenplaces_multicam.txt
"""
...
def main():
    ...
    parser.add_argument("--database", type=str,
                        default=os.path.join(MODELS_DIR, "database_eigenplaces_multicam.pkl"))
    parser.add_argument("--index", type=str,
                        default=os.path.join(MODELS_DIR, "place_index_eigenplaces_multicam.faiss"))
    ...
    # Merge candidates within merge_radius
    merged = []
